...
@endsection
@section('content')
    <x-base-content title="Data Presensi Guru" description="Data hasil presensi menggunakan wa">
        @if (Session::has('message'))
            <x-alert></x-alert>
        @endif
        <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-tab"
                data-tabs-toggle="#tab---presences--guru" role="tablist">
                <li class="me-2" role="presentation">
                    <button id="tab__presence__guru" class="inline-block p-4 border-b-2 rounded-t-lg" id="data-tab"
                        data-tabs-target="#Data" type="button" role="tab" aria-controls="Data"
                        aria-selected="{{ $tab === 'Data' ? 'true' : 'false' }}">Data</button>
                </li>
                <li class="me-2" role="presentation">
                    <button id="tab__presence__guru" class="inline-block p-4 border-b-2 rounded-t-lg" id="stats-tab"
        ...
        <div class="max-w-sm mb-3">
            ...
            <select id="change_show_rek_guru"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <div class="flex gap-1">
            <a href={{ route('presensi-guru-export', array_merge($filter, ['type' => 'excel'])) }} target="_blank" z
            ...
            <a href={{ route('presensi-guru-export', array_merge($filter, ['type' => 'pdf'])) }} target="_blank" z
...
@push('scripts')
    <script type="module">
        $(document).ready(function() {
            ...
            $("#change_show_rek_guru").on("change", function(e) {
                ...
                if (show_var === 'current') {
                    window.location.href = `{!! route('presensi-guru') !!}?show=${show_var}`
                } else {
                    window.location.href = `{!! route('presensi-guru') !!}${queryParams}&show=${show_var}`
                }
            })

            $(document).on("click", "#tab__presence__guru", function() {
                const button = $(this)
